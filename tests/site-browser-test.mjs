...
try {
  ...
  /* מה שנשמר כאן הוא ההבטחה, לא הניסוח: שהמערכת מסבירה כל
     שיבוץ. זה הבידול שבגללו מנהל סומך על סידור אוטומטי במקום
     לבנות אותו מחדש ביד, והוא חייב להופיע בפתיחה.

     ההטיה פתוחה כי הנושא יכול להיות "ה-AI מסביר" או "המערכת
     מסבירה" — שתיהן עברית תקינה, ובדיקה שנופלת על החלפה ביניהן
     בודקת את הדקדוק שלי ולא את המוצר. הסרה של ההבטחה עצמה
     עדיין מפילה אותה. */
  check('ומשפט הפתיחה מבטיח הסבר לכל שיבוץ',
    await page.locator('.lp-lead').first().textContent(), /מסביר(ה)? למה/);
